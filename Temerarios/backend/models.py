...
class Usuario (db.Model):
    ...
    contra=db.Column(db.String(50), nullable=False)
...
class Producto(db.Model):
    ...
    # Relaciones
    categoria_producto = db.relationship('CategoriaProducto', backref='producto', lazy=True)
...
class CategoriaProducto(db.Model):
    ...
    NombreCate=db.Column(db.String(80))
...
class Pago (db.Model):
    """Tabla de pago que gestionara y tendra control del pago de orden"""
    __tablename__='Pago'
    IdPago=db.Column(db.Integer, primary_key=True)
    MetodoPago=db.Column(db.String(50), nullable=False)
    IdOrden=db.Column(db.Integer, db.ForeignKey('Ordenes.IdOrden'))
    IdUsuario=db.Column(db.Integer, db.ForeignKey('Usuario.IdUsuario'))
